...
function getArticleInfo($id=0){
	global $db;                   //全局数据库连接变量
	if($id==0){                   //如果未指定id值，则从查询字符串变量中提取
		if(empty($_GET['id'])){   //如果Url参数中未包含参数id，则否回false
			return false;
		}else{
			$id = $_GET['id'];    //如果具有id参数，则赋给$id变量
		}
	}
	//调用getOneRow查询cms_article表来获取指定文章编号的详细数据
